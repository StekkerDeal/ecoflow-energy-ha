...
field's value", reusing the field numbers the state dump reads them back on:
...
# Config field numbers, the same ones the readback uses.
CONFIG_WORK_MODE = 25
...
# The hardware ceiling the device reports in `254/39 f33.11`. The app's own
# output limit is usually lower and the device clamps to it silently, without
# rejecting the frame, so it cannot be enforced here.
...
def build_task_payload(
    ...
    Grammar read off the app's own writes:
        ...
        39.1.3  enabled, 39.1.4 its inverse
        39.1.7  window: one varint, start in the low 16 bits and end in the
                high 16, both minutes since midnight
        39.1.8  charge settings: {1: serial, 2: target SoC %, 3: watts}
