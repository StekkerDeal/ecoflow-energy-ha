...
class TestStreamAC5000EntitySet:
    ...
    async def test_binary_sensor(self, hass: HomeAssistant) -> None:
        ...
        assert keys == {"backup_reserve_enabled"}
    ...
    async def test_no_switches(self, hass: HomeAssistant) -> None:
        """Backup reserve on/off was never captured, so it stays read-only."""
        assert await _setup_keys(hass, switch_setup) == set()
