...
- The EcoFlow STREAM AC 5000 can be controlled from Home Assistant: the work mode, the charge and discharge SoC limits, and a charge and discharge power setpoint. The power setpoint is the interesting one, because this device has no direct power control at all: a scheduled task is its setpoint, so the number rewrites the task the device reports, keeping whatever window was set up in the app. On a device with no task, one covering the whole day is created, since a setpoint with nothing to attach to would do nothing. Three things about this device are worth knowing before driving it from an automation or from an optimiser such as EMHASS. Whether a smart meter is linked in the EcoFlow app decides what the power setpoint means, and that is the one thing worth knowing before automating this device. With a meter linked the device runs closed loop against it and will not discharge into an export, so the setpoint acts only as a ceiling on covering house load: 1400 W requested into a house needing 200 W delivers 200 W, and enabling feed-in does not change it. With no meter linked the device runs open loop, which the app's own help text describes as any unused power flowing to the grid, and the setpoint becomes an absolute power command: unlinking a Tibber Pulse turned that same 1400 W request into a measured 1400 W discharge. An optimiser that wants to command power rather than cap it should therefore do its own metering. Zero watts means idle rather than "no setting", so it is the way to park the battery. A setpoint only acts while the device is in custom mode; in the other modes the write is accepted and ignored, and since the device reports no error, a warning goes to the log instead. And a setpoint above the output limit set in the EcoFlow app is acknowledged and then silently clamped rather than refused, which is why that limit is exposed as its own sensor for an optimiser to read and respect. Each write was confirmed on live hardware, and the frames are reproductions of the app's own, captured from its traffic and rebuilt byte for byte. Backup reserve on and off is the one setting left out: its write was never captured, so it stays a read-only sensor. (Ref #177)
- The EcoFlow STREAM AC 5000 (serial prefix `ES22`) is supported and gets 51 sensors and a binary sensor. Until now it was skipped as an unsupported device, and it needed more than a serial prefix: despite the shared product name it has nothing in common with the BK-series Stream devices at the protocol level. It sends none of their messages, and where a Stream reports each power reading as its own value, this device sends a matrix of flows between grid, battery, house and solar, nested two and three levels deep. It therefore has its own device type and parser. The mapping comes from a capture of a live unit alongside the EcoFlow app, cross-checked against an independent smart meter reading the same grid: house consumption equals the sum of the flows into the house to within 2 W across every push frame checked. Solar and per-phase smart meter readings are created only once the device actually reports them, because whether PV is wired to the EcoFlow and which meter is linked in the app are installation choices, not model differences, and an entity Home Assistant once created stays in the registry forever. Grid import and export are taken from the flow matrix rather than from the meter total, so both count in one direction only, which is what the Energy Dashboard needs. This release is read-only: the device accepts writes for work mode, the charge and discharge limits and the scheduled power, all confirmed against real firmware, but wiring them up is a separate change. It needs the EcoFlow account sign-in, since the device is not reachable through the Developer API. (Ref #177)
