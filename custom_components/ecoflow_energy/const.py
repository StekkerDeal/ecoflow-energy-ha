...
# Backup reserve on/off is config field 30. Unlike the settings below it was
# never captured from the app, so it stays a read-only binary sensor.
...
# setpoint. Bounds are the hardware ceiling the device reports in `f33.11`, not
# the app's output limit: a setpoint above that limit is acknowledged and then
# silently clamped rather than rejected, so it cannot be enforced from here.
# That limit is exposed as the Max Discharge Power sensor for an optimiser to
# read and respect.
