...
- ``f50.1.4`` signed battery power, positive is charge (-595 to +601 W).
...
Not mapped: ``f38.1`` and ``f44`` repeat pack readings `32/50` already
carries, and mapping both makes the keys flap; ``f38.1.3``/``f44.2`` look
like a cycle count but read 497, 499 and 1311 within minutes; ``f33.9``
sat at 600 throughout so the scheduled *charge* power is unconfirmed;
...
_ES22_FIELD_MAP: dict[tuple[int, int], dict[str, tuple[str, str, float]]] = {
    (254, 39): {
        ...
        # --- live battery ---
        # f50.1.5 is not a direction flag: it read 2 while discharging and
        # still 2 while charging. The sign lives in .4 alone.
        "50.1.4": ("batt_w", _TYPE_FLOAT, 1),
    },
...
# Paths that mean zero once their parent group is present in the frame.
#
# proto3 omits a zero scalar, so an edge that stops carrying power just
# disappears from `f12`; none of 1024 observed subfields was an explicit
# zero. Without this an idle battery edge would stick at its last value.
# Only when the group is present, though: 116 of 355 pushes carry `f11`
# and no `f12`, and there an absent edge means unchanged, not zero.
#
# The solar edges stay out, since zero-filling them would report the key
# on every frame and defeat their accessory gating in const.py.
