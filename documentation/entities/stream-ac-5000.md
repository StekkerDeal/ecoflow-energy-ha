...
**Totals:** 52 sensors, 1 binary sensor, 4 numbers, 1 select
...
| BMS SoC | % | diagnostic | disabled | Pack-level SoC. Runs about two points above the system value above, which is the one the app shows |
...
None. Backup reserve on and off is the one setting whose write format was never captured from the app, so it stays a read-only binary sensor rather than a control that might send something the device does not understand.
